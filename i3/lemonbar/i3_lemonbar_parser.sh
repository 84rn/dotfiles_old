...
while read -r line ; do
  case $line in
    SYS*)
      ...
      sys_arr=(${line#???})
      # date
      bat=`acpi|grep -o "[0-9]\{1,3\}%"`
      if [ ${res_w} -gt 1024 ]; then
        date="${sys_arr[0]} ${sys_arr[1]} ${sys_arr[2]}"
      else
        date="${sys_arr[1]} ${sys_arr[2]}"
      fi
      date="%{F${color_sec_b1}}${sep_left}%{F${color_icon} B${color_sec_b1}} %{T3}${icon_bat} %{F${color_icon}T-}${bat} %{F${color_icon}}${sep_l_left} %{T2}${icon_clock}%{F- T1} ${date}"
      # time
